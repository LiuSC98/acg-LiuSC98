...
auto sample_hemisphere(
  ...
  // For Problem 4, write some code below to sample hemisphere with cosine weight
  // (i.e., the sampling frequency is higher at the top)
  float z_cos_w = sqrtf(unirand.x());
  float r_cos_w = sqrtf(1.f - z_cos_w * z_cos_w);
  dir_loc = Eigen::Vector3f( // direction in normal coordinate
      r_cos_w * std::cos(phi),
      r_cos_w * std::sin(phi),
      z_cos_w);
  pdf = dir_loc.z() / float(M_PI);
...
void search_collision_in_bvh(
  ...
  if (!bvhnodes[i_bvhnode].intersect_bv(ray_org, ray_dir)) return;
